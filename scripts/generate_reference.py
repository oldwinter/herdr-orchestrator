...
def requirement_label(command: argparse.ArgumentParser, action: argparse.Action) -> str:
    for group in command._mutually_exclusive_groups:
        if group.required and action in group._group_actions:
            options = ", ".join(f"`{option_name(member)}`" for member in group._group_actions)
            return f"yes, exactly one of (mutually exclusive): {options}"
    ...
    for name, command in sorted(subcommands.choices.items()):
        ...
        if options:
            ...
            for action in options:
                required = requirement_label(command, action)
