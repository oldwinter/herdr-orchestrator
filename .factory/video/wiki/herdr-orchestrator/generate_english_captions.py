...
ENGLISH = {
    ...
    "11": "The Python control plane and npm wrapper have no third-party runtime package dependencies. External capabilities come from local command-line tools. The npm installer places workflows, profiles, and an ownership manifest into the target project while preserving user edits. Publishing starts after the complete quality gate on main. A dedicated runner only checks whether the registry lacks the version. The actual npm publish uses OIDC Trusted Publishing on a GitHub-hosted runner, without long-lived tokens or production deployment.",
    "12": "The repository currently has nineteen reachable commits, twenty-six package Python modules, and one hundred ninety-eight static test functions. Physical test lines are about zero point eight eight to one compared with Python source. Contributors run focused tests first, then just check, followed by a real doctor or read-only smoke when needed. Remember three invariants: model output is constrained, the Coordinator owns state, and current execution must produce new evidence before success is accepted.",
}
