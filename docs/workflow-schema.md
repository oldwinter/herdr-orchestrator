...
| `agent_timeout_seconds` | 10–86400 | 单次完整派发 deadline，包含 topology provisioning、agent 启动、prompt、settlement 与 receipt 验证。到期后 coordinator 停止等待并记 `herdr_timeout`，不会杀死已在跑的 Herdr agent；lease 过期后同一任务可能被重新 claim |
